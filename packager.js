...
export class Packager {
    ...
    static async test(filter, reporter = "base") {
        ...
        const find = async (dir) => (await Promise.all((await fs.readdir(dir, {withFileTypes: true}).catch(() => ([])))
            ...
            .flat(Infinity).filter(filename => !!filename && filename.match(/.*\.c?js$/));
    ...
    static async declare(src, dest, assets) {
        ...
        const bundle = await rollup.rollup({
            ...
            plugins: [generateDeclarations({moduleName: "scimmy-routers", defaultExport: "SCIMMYRouters"})],
            onwarn: (warning, warn) => (warning.code !== "CIRCULAR_DEPENDENCY" ? warn(warning) : false)
        });
        
        // Construct the bundles with specified chunks in specified formats and write to destination
        const {output: [{originalFileNames = []} = {}]} = await bundle.write({file: path.join(dest, `${entry}.d.ts`), format: "esm", plugins: [filterGeneratedBundle()]});
